...
function appp_ion_theme_setup_path() {
	// Include 'AppPresser_Ion_Theme_Setup' class file
	// Will only load if the 'AppPresser_Ion_Theme_Setup' class
	// hasn't been added in a child-theme
	return APPP_ION_PATH . 'inc/classes/AppPresser_Ion_Theme_Setup.php';
}
require_once( appp_ion_theme_setup_path() );

/**
 * Requires 'AppPresser_Ion_Theme_Setup' class to be loaded in app_theme_setup_path()
 */
class AppPresser_Ion_Theme_Functions extends AppPresser_Ion_Theme_Setup {
	 ...
	 * @since  0.0.1
	 * @return AppPresser_Ion_Theme_Functions A single instance of this class.
	 */
	...
	protected function run_classes() {
		...
		$this->Extension_Compatibility = new AppPresser_Extension_Compatibility();
	...
	public function after_setup_theme() {
		 ...
		 */
		load_theme_textdomain( 'appp_ion', APPP_ION_PATH . 'languages' );
		...
		register_nav_menus( array(
			'primary-menu' => __( 'Primary Menu', 'appp_ion' ),
			'footer-menu' => __('Footer Menu', 'appp_ion' )
		) );
	...
	public function scripts_styles() {
		...
		// Main stylesheet
		wp_enqueue_style( 'ion-style', get_stylesheet_uri(), null, filemtime( get_stylesheet_directory() . '/style.css' ) );
		...
		// Warning! Snap dragging causes a iOS copy/pasting bug in iframe
		// Left panel menu
		wp_enqueue_script( 'snap', APPP_ION_URL ."js/snap.js", array(), filemtime( get_template_directory() ."/js/snap.js" ) );

		// multi touch events
		// wp_enqueue_script( 'hammer', APPP_ION_URL ."js/hammer.min.js", array(  ) );

		wp_enqueue_script( 'appp-js', APPP_ION_URL ."js/custom$min.js", array( 'snap', 'jquery' ), filemtime( get_template_directory() . "/js/custom$min.js" ), true );

		wp_localize_script( 'appp-js', 'l10n', array(
			'back' => __( 'Back', 'appp_ion' ),
			'offline' => __( 'You appear to be offline, please try again when you are online.', 'appp_ion' )
		) );
		...
		wp_localize_script( 'appp-js', 'appp', array(
			...
			'i18n_required_comment_text' => esc_attr__( 'Please enter comment text.', 'appp_ion' ),
		) );
...
AppPresser_Ion_Theme_Functions::run();
...
function appp_remove_hook( $tag, $function_to_remove, $priority = 10 ) {
	return AppPresser_Ion_Theme_Functions::run()->remove_hook( $tag, $function_to_remove, $priority );
}
