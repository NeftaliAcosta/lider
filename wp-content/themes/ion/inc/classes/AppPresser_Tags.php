...
class AppPresser_Tags {
	...
	public static function content_nav( $nav_id ) {
		...
		<nav role="navigation" id="<?php echo esc_attr( $nav_id ); ?>" class="<?php echo $nav_class; ?> item">
			<h1 class="screen-reader-text"><?php _e( 'Post navigation', 'appp_ion' ); ?></h1>

		<?php if ( is_single() ) : // navigation links for single posts ?>

			<ul class="pager row">
			<?php previous_post_link( '<li class="col previous-link">%link</li>', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'appp_ion' ) . '</span> Previous' ); ?>
			<?php next_post_link( '<li class="col next-link">%link</li>', 'Next <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'appp_ion' ) . '</span>' ); ?>
			</ul>
		...
		<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>
			...
			<li class="previous col"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'appp_ion' ) ); ?></li>

			<?php endif; ?>

			<?php if ( get_previous_posts_link() ) : ?>
			<li class="next col"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'appp_ion' ) ); ?></li>
			<?php endif; ?>
	...
	public static function comment( $comment, $args, $depth ) {
		...
		<li id="comment-<?php comment_ID(); ?>" <?php comment_class( 'item item-text-wrap' ); ?>>
			<div class="comment-body">
				<?php _e( 'Pingback:', 'appp_ion' ); ?> <?php comment_author_link(); ?> <?php edit_comment_link( __( 'Edit', 'appp_ion' ), '<span class="edit-link">', '</span>' ); ?>
			</div>
			...
			<article id="div-comment-<?php comment_ID(); ?>" class="comment-body">
				<footer class="comment-meta">
					<div class="comment-author vcard">
						...
						<?php printf( __( '%s <span class="says">says:</span>', 'appp_ion' ), sprintf( '<cite class="fn">%s</cite>', get_comment_author_link() ) ); ?>
					</div><!-- .comment-author -->

					<div class="comment-metadata">
						<a href="<?php echo esc_url( get_comment_link( $comment->comment_ID ) ); ?>">
							<time datetime="<?php comment_time( 'c' ); ?>">
								<?php printf( _x( '%1$s at %2$s', '1: date, 2: time', 'appp_ion' ), get_comment_date(), get_comment_time() ); ?>
							</time>
						</a>
						<?php edit_comment_link( __( 'Edit', 'appp_ion' ), '<span class="edit-link">', '</span>' ); ?>
					</div><!-- .comment-metadata -->

					<?php if ( '0' == $comment->comment_approved ) : ?>
					<p class="comment-awaiting-moderation"><?php _e( 'Your comment is awaiting moderation.', 'appp_ion' ); ?></p>
					<?php endif; ?>
				</footer><!-- .comment-meta -->
	...
	public static function posted_on() {
		printf( __( 'Posted on <a href="%1$s" title="%2$s" rel="bookmark"><time class="entry-date" datetime="%3$s">%4$s</time></a><span class="byline"> by <span class="author vcard"><a class="url fn n" href="%5$s" title="%6$s" rel="author">%7$s</a></span></span>', 'appp_ion' ),
			esc_url( get_permalink() ),
			esc_attr( get_the_time() ),
			esc_attr( get_the_date( 'c' ) ),
			esc_html( get_the_date() ),
			esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ),
			esc_attr( sprintf( __( 'View all posts by %s', 'appp_ion' ), get_the_author() ) ),
			get_the_author()
		);
	}
	...
	public static function get_title( $echo = true ) {
		...
		self::$title = '';
		if ( is_category() ) :
			self::$title = sprintf( __( '%s', 'appp_ion' ), '<span>' . single_cat_title( '', false ) . '</span>' );

		elseif ( is_tag() ) :
			self::$title = sprintf( __( '%s', 'appp_ion' ), '<span>' . single_tag_title( '', false ) . '</span>' );

		elseif ( isset( $object->taxonomy ) ) :
			self::$title = sprintf( __( '%s', 'appp_ion' ), '<span>' . single_term_title( '', false ) . '</span>' );
		...
		elseif ( is_author() ) :
			...
			*/
			the_post();
			self::$title = sprintf( __( '%s', 'appp_ion' ), '<span class="vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '" title="' . esc_attr( get_the_author() ) . '" rel="me">' . get_the_author() . '</a></span>' );
		...
		elseif ( is_day() ) :
			self::$title = sprintf( __( '%s', 'appp_ion' ), '<span>' . get_the_date() . '</span>' );

		elseif ( is_month() ) :
			self::$title = sprintf( __( '%s', 'appp_ion' ), '<span>' . get_the_date( 'F Y' ) . '</span>' );

		elseif ( is_year() ) :
			self::$title = sprintf( __( '%s', 'appp_ion' ), '<span>' . get_the_date( 'Y' ) . '</span>' );

		elseif ( is_404() ) :
			self::$title = __( 'Missing', 'appp_ion' );

		elseif ( is_search() ) :
			self::$title = sprintf( __( 'Search: %s', 'appp_ion' ), '<span>' . get_search_query() . '</span>' );
