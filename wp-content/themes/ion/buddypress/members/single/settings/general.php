...
<form action="<?php echo bp_displayed_user_domain() . bp_get_settings_slug() . '/general'; ?>" method="post" class="standard-form list" id="settings-form">
	...
	<?php if ( !is_super_admin() ) : ?>
		...
		<label for="pwd" class="item item-input item-stacked-label">
			<span class="input-label"><?php _e( 'Current Password <span>(required to update email or change current password)</span>', 'appp_ion' ); ?></span>
			<input type="password" name="pwd" id="pwd" size="16" value="" class="settings-input small" /> 
		</label>

		<a href="<?php echo wp_lostpassword_url(); ?>" title="<?php esc_attr_e( 'Password Lost and Found', 'appp_ion' ); ?>"><?php _e( 'Lost your password?', 'appp_ion' ); ?></a>

	<?php endif; ?>

	<label for="email" class="item item-input item-stacked-label">
		<span class="input-label"><?php _e( 'Account Email', 'appp_ion' ); ?></span>
		<input type="text" name="email" id="email" value="<?php echo bp_get_displayed_user_email(); ?>" class="settings-input" />
	</label>

	<h4 class="padding"><?php _e( 'Change Password?', 'appp_ion' ); ?></h4>

	<label for="pass1" class="item item-input item-stacked-label">
		<span class="input-label"><?php _e( 'New password <span>(leave blank for no change)</span>', 'appp_ion' ); ?></span>
	...
	<label for="pass2" class="item item-input item-stacked-label">
		<span class="input-label"><?php _e( 'Repeat New Password', 'appp_ion' ); ?></span>
	...
	<div class="submit padding">
		<input type="submit" class="button button-block" name="submit" value="<?php esc_attr_e( 'Save Changes', 'appp_ion' ); ?>" id="submit" class="auto" />
	</div>
