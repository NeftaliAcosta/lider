...
<article id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
	...
	<footer class="entry-meta">
		<?php
			...
			$category_list = get_the_category_list( __( ', ', 'appp_ion' ) );

			/* translators: used between list items, there is a space after the comma */
			$tag_list = get_the_tag_list( '', __( ', ', 'appp_ion' ) );

			if ( ! appp_categorized_blog() ) {
				// This blog only has 1 category so we just need to worry about tags in the meta text
				if ( '' != $tag_list ) {
					$meta_text = __( 'Tags: %2$s', 'appp_ion' );
				} else {
					$meta_text = '';
				}
				...
				if ( '' != $tag_list ) {
					$meta_text = __( 'Category: %1$s Tags: %2$s', 'appp_ion' );
				} else {
					$meta_text = __( 'Category: %1$s', 'appp_ion' );
				}
