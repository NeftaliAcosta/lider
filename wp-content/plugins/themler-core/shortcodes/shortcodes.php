...
function themler_shortcodes_icon_state_style($id, $args) {
    ...
    list($main_styles, ) = ShortcodesEffects::css($id, $atts, $icon_prefix, "{selector}$icon_selector:before", substr($selector, 1));
    $result = '';
...
function themler_shortcode_align_content($atts, $content = '', $tag = '') {
    ...
    $id = ShortcodesEffects::init('bd-align-content', $atts, ShortcodesEffects::CSS_EFFECT);
    $content = ShortcodesUtility::doShortcode($content);
    ShortcodesEffects::css($id, $atts, '', '{selector}');
    ...
    $css = ShortcodesEffects::print_all_css(
...
function themler_shortcode_animation($atts, $content = '', $tag) {
    ...
    $id = ShortcodesEffects::init('bd-animation', $atts, ShortcodesEffects::HTML_EFFECT);
...
function themler_shortcode_image_scaling($atts, $content = '', $tag) {
    ...
    $id = ShortcodesEffects::init('bd-imagescaling', $atts, ShortcodesEffects::HTML_EFFECT);
    $content = ShortcodesUtility::doShortcode($content);

    list($style_css, $additional_classes, $selector) = ShortcodesEffects::css($id, ShortcodesEffects::filter($atts, '!background'), '', '{selector}');

    $target_control = ShortcodesEffects::target_control($id);
    $target_selector =  $target_control['selector'];

    $css = array();
    foreach(ShortcodesEffects::getResponsiveModes() as $mode) {
        $target_height = ShortcodesEffects::css_prop($target_control['css'], 'height', $mode);
        $target_width = ShortcodesEffects::css_prop($target_control['css'], 'width', $mode);
        $css[$mode] = '';
...
?>
    <!--[<?php echo $tag ?>]-->
        <?php echo $style_css; ?>
        <style><?php echo ShortcodesEffects::print_all_css($css, '', $selector); ?></style>
        <style>
            <?php echo ShortcodesEffects::print_all_css(ShortcodesEffects::filter($atts, 'background'), 'css', "$selector .bd-imagescaling-img"); ?>
        </style>
...
// LayoutContainer
function themler_shortcode_row($atts, $content = '', $tag = '') {
    $atts = ShortcodesUtility::atts(array(
        'collapse_spacing' => '0',
        'auto_height' => '',
        'vertical_align' => 'top',
...
ShortcodesUtility::addShortcode('row', 'themler_shortcode_row');
?>
<?php
...
function themler_shortcode_text_block($atts, $content='') {
    ...
    list($style_tag, $additional_class) = $tag === 'div'
        ? ShortcodesEffects::css($id, $atts)
        : ShortcodesEffects::css($id, $atts, '', '{selector}', '', "$tag{selector}");
...
function themler_shortcode_overlay($atts, $content = '', $tag = '') {
    ...
    $id = ShortcodesEffects::init('bd-overlay', $atts, ShortcodesEffects::CSS_EFFECT);
    $content = ShortcodesUtility::doShortcode($content);
    ShortcodesEffects::css($id, $atts, '', '{selector}');
    ...
    $atts = ShortcodesEffects::filter($atts, 'background,opacity,border-radius');
    ...
    $css = ShortcodesEffects::print_all_css(array('css' => $css), 'css', $targetControl['selector']);
    $cssBefore .= ShortcodesEffects::print_all_css($atts, 'css', "${targetControl['selector']}:before");
