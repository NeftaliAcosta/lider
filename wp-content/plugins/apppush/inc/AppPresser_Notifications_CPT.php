...
class AppPresser_Notifications_CPT {
	...
	function post_excerpt_meta_box( $post ) {
		...
		<div class="inside">
			...
			<p><small>Optional: external urls open in browser, app url opens in app. *Requires paid Pushwoosh account.</small></p>
			<?php wp_nonce_field( basename( __FILE__ ), 'apppush_nonce' ); ?>
		</div>
		<?php
	}
	...
	function save_post_meta( $post_id, $post ) {
		...
		if ( !isset( $_POST['apppush_nonce'] ) || !wp_verify_nonce( $_POST['apppush_nonce'], basename( __FILE__ ) ) )
			return $post_id;

		/* Get the posted data and sanitize it for use as an HTML class. */
		$new_meta_value = ( isset( $_POST['notify_url'] ) ? esc_url( $_POST['notify_url'] ) : '' );
